function out =EMDI(Acc,Gyr,Mag,sample_freq, TauAcc, TauMag, Zeta, accRating)
...
%    TauMag = min((validAccDataCount)/sample_freq/2,TauMag1); 
  ...
  %% obtain the quaternion by gyroscope
    ...
    if (acc(1) ~= 0 || acc(2) ~= 0 || acc(3) ~= 0)                                          % check acc to be valid
        ...
        if (abs(acc*gravref_imuframe(2:4)')< 1.0)                                                     % perform correction only if acc and gravref_imuframe do NOT coincide
    ...
    if (mag(1) ~= 0 || mag(2) ~= 0 || mag(3) ~= 0)                                          % check mag to be valid
        ...
        if (abs(mag*gravref_imuframe(2:4)')<norm(mag))                                          % if measured magnetic field is vertical, perform NO correction
            ...
            if (abs(mag_projected*magref_imuframe(2:4)')<1.0)                                         % if projected measurement and reference agree, perform NO correction
